 ...
 * @param {string} [clinicOwnerName] - The name of the clinic owner. This is optional and if provided, it will be used as the folder name in the cloud storage where the images will be stored.
 * @param {string} [clinicName] - The name of the clinic. This is optional and if provided, it will be used as the folder name in the cloud storage where the images will be stored.
 *
      ...
      (resolve, reject) => {
        cloudinary.v2.uploader
          .upload_stream(
            {
              resource_type: 'image',
              folder: `/Clinic/${clinicOwnerName}/${clinicName}`,
            },
            (error, uploadResult) => {
              if (error) {
                reject(error);
